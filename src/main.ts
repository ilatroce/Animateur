...
function normalizeAnimationEffects(data) {
    ...
    if (data.preset === 'blade-storm') {
        return {
            ...
            peakTime: roundTime(peakTime),
            endTime: roundTime(endTime),
            bladeLength: THREE.MathUtils.clamp(Number.parseFloat(data.bladeLength) || 2.8, 0.8, 5.5),
            trailLength: THREE.MathUtils.clamp(Number.parseInt(data.trailLength, 10) || 24, 8, 48),
            trailWidth: THREE.MathUtils.clamp(Number.parseFloat(data.trailWidth) || 0.52, 0.16, 1.35),
            shockwaveRadius: THREE.MathUtils.clamp(Number.parseFloat(data.shockwaveRadius) || 3.4, 1.2, 6.5),
            sparkCount: THREE.MathUtils.clamp(Number.parseInt(data.sparkCount, 10) || 96, 24, 180),
            primaryColor: normalizeColorValue(data.primaryColor, '#67f8ff'),
            secondaryColor: normalizeColorValue(data.secondaryColor, '#8b5cff'),
            accentColor: normalizeColorValue(data.accentColor, '#fff06a'),
            glowColor: normalizeColorValue(data.glowColor, '#f7fbff')
        };
    }
